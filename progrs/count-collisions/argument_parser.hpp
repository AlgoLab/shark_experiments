...
static const char *shortopts = "t:r:1:2:k:b:q:vh";
...
void parse_arguments(int argc, char **argv) {
  for (char c; (c = getopt_long(argc, argv, shortopts, longopts, NULL)) != -1; ) {
    ...
    case 't':
      ...
      if(opt::nThreads <= 0) {
        ...
        std::cerr << "count-collisions: at least 1 thread is required." << std::endl
                  << "aborting..." << std::endl;
        exit(EXIT_FAILURE);
      }
      break;
    ...
    case 'k':
      ...
      if(opt::k == 0 or opt::k > 31) {
        std::cerr << USAGE_MESSAGE;
        std::cerr << "count-collisions: k must be in the range [1, 31]." << std::endl
                  << "aborting..." << std::endl;
        exit(EXIT_FAILURE);
      }
      break;
    ...
    case 'q':
      ...
      if(mq < 0) {
        std::cerr << USAGE_MESSAGE;
        std::cerr << "count-collisions: q must be a positive value." << std::endl
                  << "aborting..." << std::endl;
        exit(EXIT_FAILURE);
      }
    ...
    case 'h':
      ...
      exit(EXIT_SUCCESS);
    default:
      std::cerr << "count-collisions : unknown argument" << std::endl;
      std::cerr << "\n" << USAGE_MESSAGE;
      exit(EXIT_FAILURE);
    }
  }
  ...
  if (opt::fasta_path == "" || opt::sample1_path == "") {
    std::cerr << "count-collisions : missing required files" << std::endl;
    std::cerr << "\n" << USAGE_MESSAGE;
    exit(EXIT_FAILURE);
  }
